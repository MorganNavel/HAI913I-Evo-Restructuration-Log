...
public class LogInserter {
    ...
    public void process(String outputDir) {
        ...
        parser.getModel().getElements(ctElement -> (ctElement instanceof CtClass<?>) && ctElement.getAnnotations().stream().anyMatch(ann -> ann.getAnnotationType().getSimpleName().equals("RestController") || ann.getAnnotationType().getSimpleName().equals("Controller"))).forEach(ctElement -> {
    ...
    private void addLoggings(CtMethod<?> method) {
        ...
        if (method.getParent(CtClass.class).getSimpleName().equals("AuthController")) {
            ...
            logBuilder.append("}\"");
            CtCodeSnippetStatement logStatement = factory.Code().createCodeSnippetStatement(String.format("logger.info(%s)", logBuilder));
        ...
        for (CtParameter<?> parameter : method.getParameters()) {
            ...
            if (paramName.equalsIgnoreCase("userId") || paramName.equalsIgnoreCase("user")) {
                ...
                logBuilder.append(",  \\\"userId\\\": \" + ").append(paramName.equalsIgnoreCase("user") ? "user.getUserId()" : "userId").append(" + \"");
                ...
                logBuilder.append("  \\\"user\\\": ").append("\\\"userDetails\\\": \" +").append(userDetails).append("+ \"");
                logBuilder.append("  \\\"operate\\\": \\\"").append(operationType).append("\\\"");
                logBuilder.append(",  \\\"productId\\\": \" + ").append(paramName.equalsIgnoreCase("product") ? "product.getProductId()" : "productId").append(" + \"");
    ...
    private void addLoggerToClass(CtClass<?> ctClass) {
        ...
        if (ctClass.getFields().stream().noneMatch(f -> f.getSimpleName().equals("logger"))) {
            ...
            CtField<?> loggerField = // Classe parent
            // Modificateurs
            // Type du champ
            // Nom du champ
            // Initialisation
            ctClass.getFactory().Field().create(ctClass, Collections.singleton(ModifierKind.PRIVATE), ctClass.getFactory().Type().createReference("org.apache.logging.log4j.Logger"), "logger", ctClass.getFactory().Code().createCodeSnippetExpression(("org.apache.logging.log4j.LogManager.getLogger(" + ctClass.getSimpleName()) + ".class)"));
    ...
    private String getRequestType(CtMethod<?> method) {
        String operation = method.getAnnotations().stream().map(ann -> ann.getAnnotationType().getSimpleName()).filter(name -> name.matches("GetMapping|PostMapping|PutMapping|PatchMapping|DeleteMapping")).findFirst().orElse("UNKNOWN");
        if (method.getParent(CtClass.class).getSimpleName().equals("AuthController")) {
            return "AUTHENTICATE";
        }
    ...
    private void addAuthentificationParams(CtMethod<?> method) {
        if (method.getParameters().stream().noneMatch(param -> "userId".equals(param.getSimpleName()))) {
            ...
            CtAnnotation<?> annotation = method.getFactory().createAnnotation(method.getFactory().createCtTypeReference(RequestParam.class));
    ...
    private void addLogForEntityList(CtMethod<?> method, Factory factory, String entityType) {
        method.getBody().getStatements().stream().filter(statement -> statement.toString().startsWith(("return " + entityType.toLowerCase()) + "Repository.findAll")).findFirst().ifPresent(returnStatement -> {
            // Create dynamic fetch statement based on the entity type
            CtCodeSnippetStatement fetchEntityStatement = factory.Code().createCodeSnippetStatement(((((("List<" + entityType) + "> ") + entityType.toLowerCase()) + "s = ") + entityType.toLowerCase()) + "Repository.findAll()");
            // Create a for loop to log each entity
            CtCodeSnippetStatement forLoopStatement = factory.Code().createCodeSnippetStatement((((((((((((((((((("for (" + entityType) + " ") + entityType.toLowerCase()) + " : ") + entityType.toLowerCase()) + "s) {\n") + "    logger.info(\"{ \\\"user\\\": \\\"\" + userId + \"\\\", \\\"userDetails\\\": \" + userRepository.findById(userId).orElse(null).toString() + \", \\\"operation\\\": \\\"READ\\\", \\\"") + entityType.toLowerCase()) + "Id\\\": \" + ") + entityType.toLowerCase()) + ".get") + entityType) + "Id() + \", ") + "\\\"") + entityType.toLowerCase()) + "Name\\\": \\\"\" + ") + entityType.toLowerCase()) + ".getName() + \"\\\" }\");\n") + "}");
            // Replace the return statement to return the fetched list
            CtCodeSnippetStatement returnEntityListStatement = factory.Code().createCodeSnippetStatement(("return " + entityType.toLowerCase()) + "s");
    ...
    private void addLogForCreateProduct(CtMethod<?> method, Factory factory) {
        method.getBody().getStatements().stream().filter(statement -> statement.toString().startsWith("return productRepository.saveAndFlush(product);")).findFirst().ifPresent(returnStatement -> {
            CtCodeSnippetStatement saveProductStatement = factory.Code().createCodeSnippetStatement("productRepository.saveAndFlush(product);");
            CtCodeSnippetStatement loggerStatement = factory.Code().createCodeSnippetStatement("logger.info(\"{ \\\"user\\\": \\\"\" + userRepository.findById(userId).orElse(null).toString() + \"\\\", \\\"operate\\\": \\\"WRITE\\\", \\\"productId\\\": \" + product.getProductId() + \", \\n  \\\"productDetails\\\": \" + productRepository.findById(product.getProductId()).orElse(null).toString() + \"}\")");
            CtCodeSnippetStatement returnEntityStatement = factory.Code().createCodeSnippetStatement("return product;");
            // Replace the existing return statement with the correct code
            method.getBody().removeStatement(returnStatement);
