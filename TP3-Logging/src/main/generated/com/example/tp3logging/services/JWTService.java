...
public class JWTService {
    ...
    public String generateToken(User user) {
        return JWT.create().withSubject(user.getUserId() + "").withClaim("mail", user.getEmail()).withIssuedAt(Instant.now()).withExpiresAt(Date.from(Instant.now().plus(30, ChronoUnit.DAYS))).withJWTId(UUID.randomUUID().toString()).sign(getAlgorithm());
    }
